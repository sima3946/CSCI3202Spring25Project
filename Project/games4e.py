...
def minmax_decision(state, game, depth):
    ...
    def max_value(state, current_depth):
        if game.terminal_test(state.board) or current_depth == 0:
...
def alpha_beta_search(state, game, depth):
    ...
    def max_value(state, alpha, beta, current_depth):
        ...
        v = float('-inf')
        for a in game.actions(state):
            v = max(v, min_value(game.result(state, a), alpha, beta, current_depth - 1))
            if v >= beta:
                return v
            alpha = max(alpha, v)
        return v
    ...
    def min_value(state, alpha, beta, current_depth):
        ...
        v = float('inf')
        for a in game.actions(state):
            v = min(v, max_value(game.result(state, a), alpha, beta, current_depth - 1))
            if v <= alpha:
                return v
            beta = min(beta, v)
        return v
